...
export default function Navbar() {
  ...
  const navItems = [
    ...
    { name: "Pages", dropdown: ["About", "FAQ", "Contact"] },
    { name: "Blog", href: "/blog" },
  ...
  return (
    <header className="fixed left-0 top-0 z-50 w-full bg-white shadow-md">
      <nav className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 md:flex-row md:items-center md:justify-between md:px-6">
        {/* Top row: Browse button (left) + Support + Auth/Profile (right) */}
        <div className="flex w-full items-center justify-between md:justify-start md:gap-10">
          ...
          {/* Mobile: Hamburger and Profile button */}
          <div className="flex items-center gap-4 md:hidden">
            ...
            <SignedOut>
              <SignInButton>
                <button className="rounded-md bg-green-700 px-3 py-1 text-white hover:bg-green-800 text-sm">
                  Sign In
                </button>
              </SignInButton>
            </SignedOut>
        ...
        {/* Desktop right section: support + auth/profile */}
